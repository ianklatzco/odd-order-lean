...
def dade (ddA : DadeHypothesis G L A) : ClassFunction ↥L →ₗ[ℂ] ClassFunction G where
  ...
  map_add' α β := by
    ext u
    change ddA.dadeFun (α + β) u = ddA.dadeFun α u + ddA.dadeFun β u
    by_cases h : ∃ a, a ∈ A ∧ u ∈ ddA.support1 a
    · simp only [dadeFun, dif_pos h, ClassFunction.add_apply]
    · simp only [dadeFun, dif_neg h, add_zero]
  map_smul' c α := by
    ext u
    change ddA.dadeFun (c • α) u = c * ddA.dadeFun α u
    by_cases h : ∃ a, a ∈ A ∧ u ∈ ddA.support1 a
...
theorem setSignalizer_image_conj [Finite G] (ddA : DadeHypothesis G L A) {x : G} (hx : x ∈ L)
    {B : Finset G} (hB : ↑B ⊆ A) :
...
theorem setProd_image_conj [Finite G] (ddA : DadeHypothesis G L A) {x : G} (hx : x ∈ L)
    {B : Finset G} (hB : ↑B ⊆ A) :
...
theorem setSignalizer_insert [Finite G] (ddA : DadeHypothesis G L A) {a : G} (ha : a ∈ A)
...
theorem mem_support1_of_mul_inv_mem_setSignalizer [Finite G] (ddA : DadeHypothesis G L A)
...
theorem exists_isSetTransversal [Finite G] (L : Subgroup G) (A : Set G) :
      ...
      { toFun := fun y => ⟨(↑y₀ : G)⁻¹ * ↑(y.1 : ↥L), (hcond y₀ y.1).mp y.2⟩
        invFun := fun w =>
          ⟨⟨(↑y₀ : G) * ↑w, mul_mem y₀.2 (setNormalizer_le w.2)⟩, (hcond y₀ _).mpr (by
            change (↑y₀ : G)⁻¹ * ((↑y₀ : G) * ↑w) ∈ setNormalizer L B₀
            rw [show (↑y₀ : G)⁻¹ * ((↑y₀ : G) * ↑w) = (↑w : G) by group]
            exact w.2)⟩
        left_inv := fun y => by
          apply Subtype.ext
          apply Subtype.ext
          change (↑y₀ : G) * ((↑y₀ : G)⁻¹ * ↑(y.1 : ↥L)) = ↑(y.1 : ↥L)
          group
        right_inv := fun w => by
          apply Subtype.ext
          change (↑y₀ : G)⁻¹ * ((↑y₀ : G) * ↑w) = (↑w : G)
          group }
    calc ((Finset.univ : Finset ↥L).filter
          (fun y : ↥L => B₀.image (fun b => ↑y * b * (↑y : G)⁻¹)
  ...
  · -- on the support: the two sides both compute to `α a`
    ...
    have hcover : calPF = T.biUnion orbBF := by
      ...
      · rintro ⟨-, hBA, hBne⟩
        ...
        change B.image (fun b => (x⁻¹ * x) * b * (x⁻¹ * x)⁻¹) = B
        rw [inv_mul_cancel]
        exact image_conj_one B
      · rintro ⟨B₀, hB₀, hBorb⟩
...
/-! ### (2.11) Restriction of the Dade hypothesis -/

namespace DadeHypothesis

variable {L : Subgroup G} {A A₁ : Set G}

/-- **Peterfalvi (2.11), first part** (`restr_Dade_hyp`): the Dade hypothesis restricts
to any `L`-stable subset `A₁ ⊆ A`, *with the same signalizer functor* (compare Coq
`restr_Dade_signalizer`, which holds there only up to `def_Dade_signalizer`; here the
signalizer is data and is preserved on the nose, so `restrict_support1` is `rfl`). -/
def restrict (ddA : DadeHypothesis G L A) (hA₁ : A₁ ⊆ A)
    (hstab : ∀ x ∈ L, ∀ a ∈ A₁, x * a * x⁻¹ ∈ A₁) : DadeHypothesis G L A₁ where
  subset := fun _ ha => ddA.subset (hA₁ ha)
  one_notMem := fun h => ddA.one_notMem (hA₁ h)
  conj_mem := hstab
  fusion := fun a ha b hb hconj => ddA.fusion a (hA₁ ha) b (hA₁ hb) hconj
  signalizer := ddA.signalizer
  signalizer_le := fun a ha => ddA.signalizer_le a (hA₁ ha)
  signalizer_conj_mem := fun a ha => ddA.signalizer_conj_mem a (hA₁ ha)
  signalizer_mul := fun a ha => ddA.signalizer_mul a (hA₁ ha)
  signalizer_coprime := fun a ha b hb => ddA.signalizer_coprime a (hA₁ ha) b (hA₁ hb)

/-- The restricted hypothesis has the same signalizers (definitionally).
Coq: `restr_Dade_signalizer`. -/
theorem restrict_signalizer (ddA : DadeHypothesis G L A) (hA₁ : A₁ ⊆ A)
    (hstab : ∀ x ∈ L, ∀ a ∈ A₁, x * a * x⁻¹ ∈ A₁) :
    (ddA.restrict hA₁ hstab).signalizer = ddA.signalizer :=
  rfl

/-- The restricted hypothesis has the same local supports (definitionally).
Coq: `restr_Dade_support1`. -/
theorem restrict_support1 (ddA : DadeHypothesis G L A) (hA₁ : A₁ ⊆ A)
    (hstab : ∀ x ∈ L, ∀ a ∈ A₁, x * a * x⁻¹ ∈ A₁) :
...
theorem restrict_dade_eq [Finite G] (ddA : DadeHypothesis G L A) (hA₁ : A₁ ⊆ A)
    (hstab : ∀ x ∈ L, ∀ a ∈ A₁, x * a * x⁻¹ ∈ A₁) {α : ClassFunction ↥L}
    (hα : α ∈ ClassFunction.supportedOn ↥L (((↑) : ↥L → G) ⁻¹' A₁)) :
    (ddA.restrict hA₁ hstab).dade α = ddA.dade α := by
  ext u
  by_cases h : ∃ a, a ∈ A₁ ∧ u ∈ ddA.support1 a
  · obtain ⟨a, ha, hu⟩ := h
    rw [(ddA.restrict hA₁ hstab).dade_apply_of_mem_support1 α ha hu,
      ddA.dade_apply_of_mem_support1 α (hA₁ ha) hu]
  · have h1 : u ∉ (ddA.restrict hA₁ hstab).support :=
      fun ⟨a, ha, hu⟩ => h ⟨a, ha, hu⟩
    rw [(ddA.restrict hA₁ hstab).dade_apply_of_notMem_support α h1]
    by_cases h2 : ∃ b, b ∈ A ∧ u ∈ ddA.support1 b
    · obtain ⟨b, hb, hub⟩ := h2
      rw [ddA.dade_apply_of_mem_support1 α hb hub]
      refine (hα ⟨b, ddA.mem_of_mem_set hb⟩ ?_).symm
      intro hbA₁
      exact h ⟨b, hbA₁, hub⟩
    · rw [ddA.dade_apply_of_notMem_support α (fun ⟨b, hb, hub⟩ => h2 ⟨b, hb, hub⟩)]

/-! ### (2.3) and the normed TI-subset corollaries (Isaacs, Lemma 7.7)

In the trivial-signalizer case the Dade lift *is* ordinary induction (`dade_eq_ind`,
Coq `Dade_Ind`), so induction from a normed TI-subset is an isometry
(`PF2.normedTI_isometry`).  This is the general form of the **seed isometry**
`ClassFunction.cfInner_ind_ind_of_malnormal` (`FrobeniusKernel.lean`, M3 Task 4):
see `PF2.normedTI_isometry_of_malnormal` for the explicit identification. -/

/-- **Peterfalvi (2.3), characterization part** (`Dade_normedTI_P`): under a Dade
hypothesis, `A` is a normed TI-subset with normalizer `L` iff `A` is nonempty and all
signalizers are trivial. -/
theorem isNormedTI_iff [Finite G] (ddA : DadeHypothesis G L A) :
    IsNormedTI A L ↔ A.Nonempty ∧ ∀ a ∈ A, ddA.signalizer a = ⊥ := by
  constructor
  · intro hti
    refine ⟨hti.nonempty, fun a ha => ?_⟩
    have hCL : ∀ u ∈ Subgroup.centralizer ({a} : Set G), u ∈ L := by
      intro u hu
      have hcomm := Subgroup.mem_centralizer_singleton_iff.mp hu
      refine (hti.conj_mem_iff ha u).mp ?_
      have huc : u * a * u⁻¹ = a := by
        rw [hcomm, mul_inv_cancel_right]
      rw [huc]
      exact ha
    rw [eq_bot_iff]
    intro x hx
    have hxC : x ∈ Subgroup.centralizer ({a} : Set G) := ddA.signalizer_le a ha hx
    have hmem : x ∈ ddA.signalizer a ⊓ (L ⊓ Subgroup.centralizer {a}) :=
      Subgroup.mem_inf.mpr ⟨hx, Subgroup.mem_inf.mpr ⟨hCL x hxC, hxC⟩⟩
    rw [ddA.signalizer_inf_eq_bot ha] at hmem
    exact hmem
  · rintro ⟨hne, htriv⟩
    refine ⟨hne, fun a ha g => ?_⟩
    constructor
    · intro hg
      obtain ⟨x, hx, hxa⟩ := ddA.fusion a ha (g * a * g⁻¹) hg (isConj_iff.mpr ⟨g, rfl⟩)
      -- `x⁻¹ g ∈ C_G(a) = C_L(a)`, hence `g ∈ L`
      have hcent : x⁻¹ * g ∈ Subgroup.centralizer ({a} : Set G) := by
        rw [Subgroup.mem_centralizer_singleton_iff]
        have h1 : (x⁻¹ * g) * a * (x⁻¹ * g)⁻¹ = a := by
          calc (x⁻¹ * g) * a * (x⁻¹ * g)⁻¹ = x⁻¹ * (g * a * g⁻¹) * x := by group
            _ = x⁻¹ * (x * a * x⁻¹) * x := by rw [hxa]
            _ = a := by group
        calc (x⁻¹ * g) * a = ((x⁻¹ * g) * a * (x⁻¹ * g)⁻¹) * (x⁻¹ * g) := by group
          _ = a * (x⁻¹ * g) := by rw [h1]
      obtain ⟨y, hy, c, hc, hyc⟩ := ddA.signalizer_mul a ha (x⁻¹ * g) hcent
      rw [htriv a ha, Subgroup.mem_bot] at hy
      have hgL : x⁻¹ * g ∈ L := by
        rw [hyc, hy, one_mul]
        exact (Subgroup.mem_inf.mp hc).1
      have := mul_mem hx hgL
      rwa [mul_inv_cancel_left] at this
    · intro hg
      exact ddA.conj_mem g hg a ha

variable (L A) in
/-- **Peterfalvi (2.3), existence part** (`normedTI_Dade`): a normed TI-subset avoiding
`1` satisfies the Dade hypothesis, with trivial signalizers. -/
def ofIsNormedTI (h1 : (1 : G) ∉ A) (hti : IsNormedTI A L) : DadeHypothesis G L A where
  subset := hti.subset
  one_notMem := h1
  conj_mem := fun x hx a ha => hti.conj_mem hx ha
  fusion := fun a ha b hb hconj => by
    obtain ⟨c, hc⟩ := isConj_iff.mp hconj
    refine ⟨c, (hti.conj_mem_iff ha c).mp ?_, hc⟩
    rw [hc]
    exact hb
  signalizer := fun _ => ⊥
  signalizer_le := fun _ _ => bot_le
  signalizer_conj_mem := fun a _ g _ x hx => by
    rw [Subgroup.mem_bot] at hx ⊢
    rw [hx]
    group
  signalizer_mul := fun a ha u hu => by
    refine ⟨1, one_mem _, u, Subgroup.mem_inf.mpr ⟨?_, hu⟩, (one_mul u).symm⟩
    have hcomm := Subgroup.mem_centralizer_singleton_iff.mp hu
    refine (hti.conj_mem_iff ha u).mp ?_
    have huc : u * a * u⁻¹ = a := by
      rw [hcomm, mul_inv_cancel_right]
    rw [huc]
    exact ha
  signalizer_coprime := fun _ _ _ _ => by
    rw [Subgroup.card_bot]
    exact Nat.coprime_one_left _

/-- **`Dade_Ind`** (supplement to Peterfalvi (2.3)/(2.6)(a); implies Isaacs, Lemma 7.7):
over a normed TI-subset the Dade lift coincides with ordinary induction on `'CF(L, A)`. -/
theorem dade_eq_ind [Fintype G] (ddA : DadeHypothesis G L A)
    (hti : IsNormedTI A L) {α : ClassFunction ↥L}
    (hα : α ∈ ClassFunction.supportedOn ↥L (((↑) : ↥L → G) ⁻¹' A)) :
    ddA.dade α = ClassFunction.ind L α := by
  letI : Fintype ↥L := Fintype.ofFinite ↥L
  have hsig := (ddA.isNormedTI_iff.mp hti).2
  refine ClassFunction.eq_of_forall_cfInner_irr_eq fun χ => ?_
  rw [ClassFunction.cfInner_ind_eq_cfInner_res]
  refine ddA.dade_reciprocity hα (χ : ClassFunction G) fun a ha x hx => ?_
  rw [hsig a ha, Subgroup.mem_bot] at hx
  rw [hx, one_mul]

end DadeHypothesis

/-! ### Induction from a normed TI-subset (Isaacs, Lemma 7.7) -/

section NormedTICorollaries

variable {L : Subgroup G} {A : Set G} [Fintype G] [Fintype L]

omit [Fintype L] in
/-- **`normedTI_Ind_id1`/`normedTI_Ind_id`** (the identity part of Isaacs, Lemma 7.7):
for a normed TI-subset `A ∌ 1` and `α ∈ 'CF(L, A)`, `'Ind_L^G α` agrees with `α` on
`{1} ∪ A`. -/
theorem normedTI_ind_apply (h1 : (1 : G) ∉ A) (hti : IsNormedTI A L)
    {α : ClassFunction ↥L}
    (hα : α ∈ ClassFunction.supportedOn ↥L (((↑) : ↥L → G) ⁻¹' A)) {a : G} (haL : a ∈ L)
    (ha : a = 1 ∨ a ∈ A) : ClassFunction.ind L α a = α ⟨a, haL⟩ := by
  rw [← (DadeHypothesis.ofIsNormedTI L A h1 hti).dade_eq_ind hti hα]
  exact (DadeHypothesis.ofIsNormedTI L A h1 hti).dade_apply_of_mem_insert hα haL ha

/-- **`normedTI_isometry`** (the isometry part of Isaacs, Lemma 7.7): induction from a
normed TI-subset `A ∌ 1` is an isometry on `'CF(L, A)`.  Derived from the Dade isometry
via `Dade = Ind`. -/
theorem normedTI_isometry (h1 : (1 : G) ∉ A) (hti : IsNormedTI A L)
    {α β : ClassFunction ↥L}
    (hα : α ∈ ClassFunction.supportedOn ↥L (((↑) : ↥L → G) ⁻¹' A))
    (hβ : β ∈ ClassFunction.supportedOn ↥L (((↑) : ↥L → G) ⁻¹' A)) :
    ⟪ClassFunction.ind L α, ClassFunction.ind L β⟫_[G] = ⟪α, β⟫_[↥L] := by
  rw [← (DadeHypothesis.ofIsNormedTI L A h1 hti).dade_eq_ind hti hα,
    ← (DadeHypothesis.ofIsNormedTI L A h1 hti).dade_eq_ind hti hβ]
  exact (DadeHypothesis.ofIsNormedTI L A h1 hti).dade_isometry hα β

/-- **The seed identification**: for a nontrivial malnormal subgroup `H`, the normed
TI-subset `A = H \ {1}` (bridge `PF2.isNormedTI_sdiff_one_of_malnormal`) recovers the
TI-induction isometry of `FrobeniusKernel.lean` from the Dade machinery: induction is an
isometry on the class functions of `H` vanishing at `1`.  This is the statement of the
M3 seed `ClassFunction.cfInner_ind_ind_of_malnormal` with both arguments vanishing at
`1` (the seed itself is one-sided-stronger — only `α 1 = 0` is needed there — and is
*not* re-proved here; MathComp `normedTI_isometry` corresponds to the general
`PF2.normedTI_isometry` above). -/
theorem normedTI_isometry_of_malnormal {H : Subgroup G} [Fintype H] (hbot : H ≠ ⊥)
    (hmal : ∀ g ∉ H, H ⊓ H.map (MulAut.conj g).toMonoidHom = ⊥)
    {α β : ClassFunction ↥H} (hα1 : α 1 = 0) (hβ1 : β 1 = 0) :
    ⟪ClassFunction.ind H α, ClassFunction.ind H β⟫_[G] = ⟪α, β⟫_[↥H] := by
  have hsupp : ∀ {φ : ClassFunction ↥H}, φ 1 = 0 →
      φ ∈ ClassFunction.supportedOn ↥H (((↑) : ↥H → G) ⁻¹' ((H : Set G) \ {1})) := by
    intro φ hφ1 h hh
    have hne : (↑h : G) ∉ (H : Set G) \ {1} := hh
    have h1 : (↑h : G) = 1 := by
      by_contra hcon
      exact hne ⟨h.2, hcon⟩
    have : h = 1 := Subtype.ext h1
    rwa [this]
  exact normedTI_isometry (by simp) (isNormedTI_sdiff_one_of_malnormal hbot hmal)
    (hsupp hα1) (hsupp hβ1)

/-- Cross-check: the seed isometry `ClassFunction.cfInner_ind_ind_of_malnormal` proves
the same statement (with its weaker one-sided hypothesis). -/
example {H : Subgroup G} [Fintype H] (_hbot : H ≠ ⊥)
    (hmal : ∀ g ∉ H, H ⊓ H.map (MulAut.conj g).toMonoidHom = ⊥)
    {α β : ClassFunction ↥H} (hα1 : α 1 = 0) (_hβ1 : β 1 = 0) :
    ⟪ClassFunction.ind H α, ClassFunction.ind H β⟫_[G] = ⟪α, β⟫_[↥H] :=
  ClassFunction.cfInner_ind_ind_of_malnormal hmal hα1 β

end NormedTICorollaries
