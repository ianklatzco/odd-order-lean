...
| `cfcenter` (`character.v`) | `Irr.cfcenter` (the modulus-equality set `{g | ‖χ g‖ = χ 1}`; a subgroup via the scalar-action predicate `Irr.IsScalarOn`; `Irr.norm_apply_eq_of_mem_center` = `Z(G) ≤ Z(χ)`) | `OddOrder/Mathlib/RepresentationTheory/CharCenter.lean` |
| `irr1_bound` (`character.v`) | `Irr.irr1_bound` (`χ(1)² ≤ |G:Z(χ)|`; reusable counting core `Irr.sq_re_le_index_of_forall_norm_eq`) | `OddOrder/Mathlib/RepresentationTheory/CharCenter.lean` |
...
| `irr1_bound_quo` (Peterfalvi (1.8); `PFsection1.v`) | `PF1.irr1_bound_quo` (fully proved, sorry-free; leaf `PF1.irr1_bound_charCenter` closed via `Irr.irr1_bound`/`Irr.cfcenter`) | `OddOrder/PF/Section1.lean` |
...
| `cfMorph` (`classfun.v`; pullback of class functions along a morphism) | `ClassFunction.compHom` (bundled `→ₗ[ℂ]`; `cfMorph_char`: `ClassFunction.IsChar.compHom`) | `OddOrder/Mathlib/RepresentationTheory/CharacterTransfer.lean` |
| `cfAut` (`classfun.v`; pointwise ring endomorphism of the values) | `ClassFunction.aut` (`conjC` case: `ClassFunction.aut_starRingEnd`) | `OddOrder/Mathlib/RepresentationTheory/CharAut.lean` |
| `cfInd_vchar` (`vcharacter.v`; usage PFsection2.v:728) | `ClassFunction.IsVirtualChar.ind` | `OddOrder/Mathlib/RepresentationTheory/VirtualChar.lean` |
| `partition_cent_rcoset` (Peterfalvi (2.1); `PFsection2.v`) | restated as its consumed consequences: cover `PF2.exists_conj_mul_of_mul_inv_mem`, trivial intersection `PF2.mem_centralizer_of_conj_mul_mem`, block count `PF2.DadeHypothesis.card_filter_conj_coset_mul`; the `π`-part calculus `u.`_π = g` becomes the CRT exponent `PF2.exists_pow_eq_coprime_mul` | `OddOrder/PF/Section2.lean` |
| `normedTI` (`frobenius.v`, via `normedTI_memJ_P`; ambient `G` = whole group) | `PF2.IsNormedTI` (subset ↔ malnormal-subgroup bridge: `PF2.isNormedTI_sdiff_one_of_malnormal` / `PF2.malnormal_of_isNormedTI_sdiff_one`) | `OddOrder/PF/Section2.lean` |
| `Dade_hypothesis` (Peterfalvi Definition (2.2); `PFsection2.v`) + `Dade_signalizer` + `is_Dade_signalizer` | `PF2.DadeHypothesis` (structure carrying the signalizer as data; `H a ⋊ C_L(a) = C_G(a)` split into the `signalizer_le`/`signalizer_conj_mem`/`signalizer_mul` fields) | `OddOrder/PF/Section2.lean` |
| `def_Dade_signalizer` (uniqueness of the signalizer) | `PF2.DadeHypothesis.signalizer_eq_of` (any two hypothesis structures agree on `A`; Hall-containment engine `PF2.DadeHypothesis.mem_signalizer_of_coprime` replaces `sub_normal_Hall`/`pcore` canonicity) | `OddOrder/PF/Section2.lean` |
| `Dade_sdprod` / `Dade_coprime` | `PF2.DadeHypothesis.isComplement'_subgroupOf` + `normal_signalizer_subgroupOf` + `card_signalizer_mul_card` / `signalizer_coprime` (field) | `OddOrder/PF/Section2.lean` |
| `Dade_normedTI_P` (Peterfalvi (2.3), characterization; `PFsection2.v`) | `PF2.DadeHypothesis.isNormedTI_iff` | `OddOrder/PF/Section2.lean` |
| `DadeJ` (Peterfalvi (2.4)(a); `PFsection2.v`) | `PF2.DadeHypothesis.signalizer_conj` | `OddOrder/PF/Section2.lean` |
| `Dade_support1` / `mem_Dade_support1` / `Dade_support1_id` | `PF2.DadeHypothesis.support1` / `mul_mem_support1` / `support1_conj` | `OddOrder/PF/Section2.lean` |
| `Dade_support1_TI` (Peterfalvi (2.4)(b); `PFsection2.v`) | `PF2.DadeHypothesis.exists_conj_of_mem_support1` (fiber form: `mem_support1_iff_of_mem`) | `OddOrder/PF/Section2.lean` |
| `Dade_cover_TI` / `norm_Dade_cover` (Peterfalvi (2.4)(c), strengthened; `PFsection2.v`) | `PF2.DadeHypothesis.mem_centralizer_of_conj_coset` (normalizer count: `card_filter_conj_mem_coset`) | `OddOrder/PF/Section2.lean` |
| `Dade_support` / `not_support_Dade_1` / `Dade_support_normal` | `PF2.DadeHypothesis.support` / `one_notMem_support` / `conj_mem_support` | `OddOrder/PF/Section2.lean` |
| `Dade` + `Dade_is_linear` (Peterfalvi Definition (2.5); `PFsection2.v`) | `PF2.DadeHypothesis.dade : ClassFunction ↥L →ₗ[ℂ] ClassFunction G` (total, as in the Coq) | `OddOrder/PF/Section2.lean` |
| `DadeE` (validity of Definition (2.5)) | `PF2.DadeHypothesis.dade_apply_of_mem_support1` | `OddOrder/PF/Section2.lean` |
| `Dade_id` / `Dade_cfunS` / `Dade_cfun` / `Dade1` / `Dade_id1` | `PF2.DadeHypothesis.dade_apply_of_mem` / `dade_mem_supportedOn` / `dade_mem_supportedOn_compl_one` / `dade_apply_one` / `dade_apply_of_mem_insert` | `OddOrder/PF/Section2.lean` |
| `Dade_aut` / `Dade_conjC` (`PFsection2.v`) | `PF2.DadeHypothesis.dade_aut` / `dade_conjC` | `OddOrder/PF/Section2.lean` |
| `general_Dade_reciprocity` (Peterfalvi (2.7), main part; `PFsection2.v`) | `PF2.DadeHypothesis.general_dade_reciprocity` (helper counts: `sum_conj_coset`, `sum_card_inf_centralizer`) | `OddOrder/PF/Section2.lean` |
| `Dade_reciprocity` (Peterfalvi (2.7), second part) | `PF2.DadeHypothesis.dade_reciprocity` | `OddOrder/PF/Section2.lean` |
| `Dade_isometry` (Peterfalvi (2.6)(a); `PFsection2.v`) | `PF2.DadeHypothesis.dade_isometry` (one-sided support hypothesis — only the first argument need vanish off `A`) | `OddOrder/PF/Section2.lean` |
| `Dade_set_signalizer` (`'H(B)`) / `Dade_set_normalizer` (`'M(B)`) / `'N_L(B)` | `PF2.DadeHypothesis.setSignalizer` / `PF2.DadeHypothesis.setProd` / `PF2.setNormalizer` | `OddOrder/PF/Section2.lean` |
| `Dade_set_sdprod` (Peterfalvi (2.8); `PFsection2.v`) | `PF2.DadeHypothesis.exists_mul_of_mem_setProd` + `isComplement'_setProd` + `normal_setSignalizer_subgroupOf'` + `setSignalizer_inf_setNormalizer` + `card_setProd` | `OddOrder/PF/Section2.lean` |
| `Dade_restrm` / `Dade_cfun_restriction` / `Dade_restrictionE` (`'aa_B`; `PFsection2.v`) | `PF2.DadeHypothesis.dadeProj` (projection `'M(B) →* L`, junk `1` off `calP` like the Coq `trivm`) / `dadeRestriction` / `dadeRestriction_apply_mul` | `OddOrder/PF/Section2.lean` |
| `Dade_restriction_vchar` (`PFsection2.v`) | `PF2.DadeHypothesis.dadeRestriction_isVirtualChar` | `OddOrder/PF/Section2.lean` |
| `Dade_Ind_restr_J` (Peterfalvi (2.10.1); `PFsection2.v`) | `PF2.DadeHypothesis.ind_dadeRestriction_image_conj` | `OddOrder/PF/Section2.lean` |
| `Dade_setU1` (Peterfalvi (2.10.2); `PFsection2.v`) | `PF2.DadeHypothesis.setSignalizer_insert` | `OddOrder/PF/Section2.lean` |
| `Dade_Ind_expansion` (Peterfalvi (2.10.3); `PFsection2.v`) | `PF2.DadeHypothesis.ind_dadeRestriction_apply_of_notMem` (first clause) / `ind_dadeRestriction_apply_of_mem` (second clause; coset engine `mem_support1_of_mul_inv_mem_setSignalizer`) | `OddOrder/PF/Section2.lean` |
| `Dade_transversal` (`PFsection2.v`; local to the file in the Coq) | `PF2.IsSetTransversal` (predicate) + `PF2.exists_isSetTransversal` (statement-shape deviation: (2.10) is stated for an arbitrary transversal, see the module docstring) | `OddOrder/PF/Section2.lean` |
| `Dade_expansion` (Peterfalvi (2.10); `PFsection2.v`) | `PF2.DadeHypothesis.dade_expansion` | `OddOrder/PF/Section2.lean` |
| `Dade_vchar` (Peterfalvi (2.6)(b); `PFsection2.v`) | `PF2.DadeHypothesis.dade_isVirtualChar` (bundled `'Z[irr G, G^#]` form: `dade_mem_virtualChar_compl_one`) | `OddOrder/PF/Section2.lean` |
| `Dade_Zisometry` (summary of Peterfalvi (2.6); `PFsection2.v`) | `PF2.DadeHypothesis.dade_Zisometry` | `OddOrder/PF/Section2.lean` |
| `restr_Dade_hyp` / `restr_Dade_signalizer` / `restr_Dade_support1` / `restr_DadeE` (Peterfalvi (2.11); `PFsection2.v`) | `PF2.DadeHypothesis.restrict` / `restrict_signalizer` (`rfl`) / `restrict_support1` (`rfl`) / `restrict_dade_eq` | `OddOrder/PF/Section2.lean` |
| `normedTI_Dade` (existence part of Peterfalvi (2.3); `PFsection2.v`) | `PF2.DadeHypothesis.ofIsNormedTI` | `OddOrder/PF/Section2.lean` |
| `Dade_Ind` (`PFsection2.v`; with Isaacs, Lemma 7.7) | `PF2.DadeHypothesis.dade_eq_ind` | `OddOrder/PF/Section2.lean` |
| `normedTI_Ind_id1` / `normedTI_Ind_id` (`PFsection2.v`) | `PF2.normedTI_ind_apply` (both, via the `a = 1 ∨ a ∈ A` hypothesis) | `OddOrder/PF/Section2.lean` |
| `normedTI_isometry` (`PFsection2.v`; isometry part of Isaacs, Lemma 7.7) | `PF2.normedTI_isometry`; **seed identification**: the M3 seed `ClassFunction.cfInner_ind_ind_of_malnormal` (`FrobeniusKernel.lean`) is the malnormal-subgroup case `A = H \ {1}`, recovered as `PF2.normedTI_isometry_of_malnormal` (the seed keeps its one-sided-vanishing strengthening) | `OddOrder/PF/Section2.lean` |
...
| `normedTI_isometry`-correspondent (approximate: the trivial-intersection seed of the Dade isometry; the Coq lemma is stated for `normedTI` configurations — **since M6 Task 3 the general form is `PF2.normedTI_isometry`**, and the seed is its malnormal-subgroup case `A = H \ {1}`, see `PF2.normedTI_isometry_of_malnormal`) | `ClassFunction.cfInner_ind_ind_of_malnormal` (`⟪ind α, ind β⟫_[G] = ⟪α, β⟫_[H]` for `α 1 = 0`, `H` malnormal; via `ClassFunction.res_ind_eq_self_of_malnormal`) | `OddOrder/Mathlib/GroupTheory/FrobeniusKernel.lean` |
